...
export default function Sidebar({ activePage, onNavigate, isDark, onToggleTheme, collapsed }: Props) {
  return (
    <aside style={{
      ...
      {/* Logo */}
      <div style={{ padding: collapsed ? "14px 0" : "14px 14px", display: "flex", alignItems: "center", gap: 10, borderBottom: "1px solid var(--sidebar-border)", justifyContent: collapsed ? "center" : "flex-start", flexShrink: 0 }}>
      ...
      <nav style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: collapsed ? "8px 4px" : "8px 8px" }}>
        {sections.map((sec, si) => (
          <div key={si} style={{ marginBottom: 4 }}>
            ...
            {collapsed && si > 0 && <div style={{ height: 1, background: "var(--sidebar-border)", margin: "6px 4px" }} />}
            {sec.items.map(item => {
      ...
      {/* Footer */}
      <div style={{ borderTop: "1px solid var(--sidebar-border)", padding: collapsed ? "8px 4px" : "8px 8px", flexShrink: 0 }}>
        <div
          className="sidebar-item"
          onClick={onToggleTheme}
          style={{ justifyContent: collapsed ? "center" : "flex-start", padding: collapsed ? "8px 6px" : "7px 10px" }}
          title={collapsed ? (isDark ? "Light mode" : "Dark mode") : undefined}
        >
          {isDark ? <Sun size={15} strokeWidth={2} style={{ flexShrink: 0 }} /> : <Moon size={15} strokeWidth={2} style={{ flexShrink: 0 }} />}
          {!collapsed && <span style={{ fontSize: 12.5 }}>{isDark ? "Light Mode" : "Dark Mode"}</span>}
        </div>
